...
function initDragDropHandler(imgContainer) {
    ...
    // 移动端触摸上传支持
    initMobileTouchUpload(imgContainer)
}

/**
 * 初始化移动端触摸上传功能
 */
function initMobileTouchUpload(imgContainer) {
    ...
    imgContainer.addEventListener('touchstart', (e) => {
        ...
        // 显示长按提示
        imgContainer.classList.add('touch-hint')
    }, { passive: false })
    ...
    imgContainer.addEventListener('touchmove', (e) => {
        ...
        if (Math.abs(currentY - touchStartY) > 50) {
            imgContainer.classList.remove('touch-hint')
        }
    }, { passive: false })
    ...
    imgContainer.addEventListener('touchcancel', (e) => {
        e.preventDefault()
        imgContainer.classList.remove('touch-hint')
    }, { passive: false })
}
...
function initRadioOptionsHandler() {
    ...
    radioOptions.forEach(option => {
        ...
        // 触摸取消恢复
        option.addEventListener('touchcancel', function() {
            this.style.transform = 'scale(1)'
        })
